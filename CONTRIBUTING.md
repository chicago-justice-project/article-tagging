...
If Python is installed on your system uv will detect and use it.
If not, uv will download Python automatically.

```
git clone https://github.com/chicago-justice-project/article-tagging.git
cd article-tagging
```

### Get the required data

Download the [Natural Language Toolkit (NLTK)](http://www.nltk.org/) data:

```
uv run python -c "import nltk; nltk.download('punkt_tab', '.venv/nltk_data')"
uv run python -c "import nltk; nltk.download('wordnet', '.venv/nltk_data')"
```

For the geotagging model you will need the [GloVe](https://nlp.stanford.edu/projects/glove/) pre-trained word vectors:

```
curl -O https://downloads.cs.stanford.edu/nlp/data/glove.6B.zip
unzip glove.6B.zip -d lib/tagnews/data
```

You will also need the [Chicago Community Areas boundary data](https://data.cityofchicago.org/d/igwz-8jzy):

```
curl https://data.cityofchicago.org/api/geospatial/igwz-8jzy?method=export&format=GeoJSON \
    -o "lib/tagnews/data/Boundaries - Community Areas (current).geojson"
```

The latest data dump from the [Chicago Justice Project](https://github.com/chicago-justice-project/chicago-justice) should be placed in `lib/tagnews/data`.
If you do not have access to the production data you can use the test dataset included in the repo:

```
cp lib/tagnews/data/ci-data/*.csv lib/tagnews/data
```
...
uv run python -m tagnews.geoloc.models.lstm.save_model
```
...
The `save_model.py` can be run as a script to save the trained model:
...
The `save_model.py` file can be run as a script to save the trained model:
...
You need the data to run the tests. If you have the data, great! You should be able to run the tests. If you don't have the data, you use the tiny subset of the data stored in `lib/tagnews/data/ci-data/`.

```
cp lib/tagnews/data/ci-data/*.csv lib/tagnews/data
```

Make sure you have downloaded the GloVE vectors (see Getting Started above).
